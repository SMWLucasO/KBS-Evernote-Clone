    ...
    public class NoteLabelRepository : IRepository<NoteLabelModel>
    {
        ...
        public bool Insert(NoteLabelModel toInsert)
        {
            if (toInsert != null)
            {
                ...
                List<NoteLabelModel> noteLabels = GetBy(new[] {"NoteID = @NoteID", "LabelID = @LabelID"}, parameters).ToList();
                if (noteLabels.Count > 0)
                    return true;

                return DataAccess.Instance.Execute("INSERT INTO [NoteLabel] ([NoteID], [LabelID]) VALUES (@NoteID, @LabelID)", parameters);
            }
            return false;
        }
        ...
        public bool Update(NoteLabelModel toUpdate) => false;
        ...
        public bool Delete(NoteLabelModel toDelete)
        {
            if (toDelete != null)
            {
                ...
                return DataAccess.Instance.Execute("DELETE FROM [NoteLabel] WHERE NoteID = @NoteID AND LabelID = @LabelID", parameters);
            }
